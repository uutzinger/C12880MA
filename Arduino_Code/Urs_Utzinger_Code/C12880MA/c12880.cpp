...
void C12880::begin() {
  ...
  _adc->disablePGA(ADC_0);
  _adc->setReference(ADC_REFERENCE::REF_3V3, ADC_0); //For Teensy 3.x ADC_REF_INTERNAL is 1.2V, default is 3.3V
  _adc->setAveraging(SAMPLE_AVERAGING); // set number of averages
  _adc->setResolution(SAMPLE_RESOLUTION); // set bits of resolution
  // it can be VERY_LOW_SPEED, LOW_SPEED, MED_SPEED, HIGH_SPEED_16BITS, HIGH_SPEED or VERY_HIGH_SPEED
  _adc->setConversionSpeed(ADC_CONVERSION_SPEED::VERY_HIGH_SPEED); // change the conversion speed
  // it can be VERY_LOW_SPEED, LOW_SPEED, MED_SPEED, HIGH_SPEED or VERY_HIGH_SPEED
  _adc->setSamplingSpeed(ADC_SAMPLING_SPEED::VERY_HIGH_SPEED);    // change the sampling speed, we can use high speed since we have buffer
