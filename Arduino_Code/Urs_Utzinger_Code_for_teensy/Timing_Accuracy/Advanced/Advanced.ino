...
void setup() {
  ///// ADC0 ////--------------------------------------------------------------
  adc->adc0->setReference(ADC_REFERENCE::REF_3V3); // For Teensy 3.x ADC_REF_INTERNAL is 1.2V, default is 3.3V
  ...
  adc->adc0->setSamplingSpeed(ADC_SAMPLING_SPEED::VERY_HIGH_SPEED);    // change the sampling speed, we can use high speed since we have buffer
...
void loop() {
  ...
  for(int i = 0; i < C12880_NUM_CHANNELS; i++){
    ...
    while(!adc->adc0->isComplete()){};                  //poll for completion
    data[i] = (uint16_t) adc->adc0->analogReadContinuous();
  }
  adc->adc0->stopContinuous();
   ...
   for(int i = 0; i < C12880_NUM_CHANNELS; i++){
     data[i] = adc->adc0->analogRead(VIDEO_pin);
     pulse_clock(1);
   }
  #endif
  timings[4] = micros();
